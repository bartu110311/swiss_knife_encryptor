...
def _derive_key(password: str, salt: bytes) -> bytes:
    # 256-bit (32 bytes) anahtar üretiyoruz
    kdf = PBKDF2HMAC(
...
def encrypt_text(text: str, password: str) -> str:
    salt = os.urandom(16)
    iv = os.urandom(8)  # Blowfish block size = 64 bits = 8 bytes
    key = _derive_key(password, salt)
    
    # Uyarı vermemesi için decrepit_algos.Blowfish kullanıyoruz
    cipher = Cipher(decrepit_algos.Blowfish(key), modes.CBC(iv), backend=default_backend())
...
def encrypt_file(filepath: str, out_path: str, password: str):
    ...
    print(f"[+] File encrypted successfully with Blowfish: {out_path}")
...
def decrypt_file(filepath: str, out_path: str, password: str):
    ...
    with open(out_path, 'wb') as f:
        f.write(plaintext)
    print(f"[+] File decrypted successfully with Blowfish: {out_path}")
...
def encrypt_text(text: str, password: str, bits: int = 256) -> str:
    ...
    cipher = Cipher(algorithms.AES(key), modes.OFB(nonce), backend=default_backend())
...
def encrypt_file(filepath: str, out_path: str, password: str, bits: int = 256):
    ...
    print(f"[+] File encrypted successfully with AES-OFB: {out_path}")
...
def decrypt_file(filepath: str, out_path: str, password: str, bits: int = 256):
    ...
    with open(out_path, 'wb') as f:
        f.write(plaintext)
    print(f"[+] File decrypted successfully with AES-OFB: {out_path}")